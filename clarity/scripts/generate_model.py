...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Generates a data extensions model from a Clarity CodeQL database"
    )
    parser.add_argument("database_path", help="filepath to a Clarity CodeQL database")
    parser.add_argument(
        "-o",
        "--output",
        required=False,
...
def check_database_exists(database_path):
    if not database_path.exists():
        ...
        die("directory: " + str(database_path) + " doesn't look like a Clarity database")
...
def serialize_type(tuples, extensible_type):
    return {
        "addsTo": {"pack": "codeql/clarity-all", "extensible": extensible_type},
        "data": tuples,
    }
